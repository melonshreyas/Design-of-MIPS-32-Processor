...
print "Update in Input List because of mixture of Multibit and Singlebit Input\n";
...
rename ("$Dirname/Testbench.v", "$Dirname/$New_name.v") or die "Error in renaming : $!";
...
{
push (@Range_UL_Input,2**$Input_Multi_MSB);
...
my @Monitor_List = @_;
my $Total_Port_Count = @Monitor_List;
print"Monitor_List =@Monitor_List\n";
print"Total Ports present in Design Module = \t\t$Total_Port_Count\n";
print"*#@#\$\**#@#\$\**#@#\$\**#@#\$\**#@#\$\**#@#\$\**#@#\$\*";
print"Problem in Backapace Character when saving it in .v Format\n";
print"No Problem of Backspace Character in Terminal Window\n";
print"\\b stands for 0X08 in ASCII. It Saves ASCII value instead of removing a character\n";
